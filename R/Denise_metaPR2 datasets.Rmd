...
environment_table <- environment_datasets %>% 
  count(ecosystem, substrate_type) 
environment_table

study_area_dataset <- environment_datasets %>%
  filter(dataset_id != 1,2,3,15,34,35,50,58)
  
study_area_map <- ggplot() + 
    geom_polygon(data = world, aes(x=long, y = lat, group = group), fill="grey") + 
    coord_fixed(1.3) +
    geom_rect(data=environment_datasets, aes(xmin=long_min, xmax=long_max, ymin=lat_min, ymax=lat_max, alpha=0.2))+
    scale_size_area(trans="log10") +
