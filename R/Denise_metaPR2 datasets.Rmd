...
table_datasets <- datasets_published %>% 
  select(gene, gene_region, dataset_id, region, ecosystem, substrate_type, bioproject_accession, paper_doi) %>% 
...
# Denise - Ecosystem and substrate

```{r}
environment_datasets <- datasets_published %>% 
  select(dataset_id, region, ecosystem, substrate_type, lat_min, lat_max, long_min, long_max)%>%
  filter(ecosystem != "granite rock basins")
environment_datasets

# ERROR- some rows are split into two, my code does not work to rename
ecosystem_table <- environment_datasets %>%
  separate_rows(ecosystem, sep = ",") %>%
  #rownames(ecosystem_table)[rownames(ecosystem_table) == " oceanic"] = "oceanic" %>%
  #rename(" coastal" = "coastal") %>%
  count(ecosystem) %>%
  arrange(desc(n))
ecosystem_table

# ERROR- some rows are split into two, my code does not work to rename
substrate_table <- environment_datasets %>%
...
# Denise - map for samples with coordinates and study area that do not have sample coordinates. Size = area sampled/number of samples collected.
```{r}

# Get dataset id only for those datasets with coordinates for samples
id_samples <- metapr2_18S %>%
  distinct(dataset_id)
id_samples

# Only select for dataset id that is not represented in samples 
study_area_dataset <- anti_join(environment_datasets, id_samples)

# Map of study area - represented by point in the middle of polygon. Size = area sampled. Is there a better way to show the map?
study_area_map <- ggplot() + 
    geom_polygon(data = world, aes(x=long, y = lat, group = group), fill="grey") + 
    coord_fixed(1.3) +
    geom_rect(data=study_area_dataset, aes(xmin=long_min, xmax=long_max, ymin=lat_min, ymax=lat_max, alpha=0.2))+
    geom_point(data=asv_summary, aes(x=longitude, y=latitude, size=n_samples), fill="blue", shape=21)+
    scale_size_area(trans="log10") +
    ggtitle("MetaPR2 study site")

study_area_map
```


# Denise - Data availability
```{r}
data_availability <- datasets_published %>% 
  select(data_available)%>%
  count(data_available)%>%
  mutate(total = sum(n))%>%
  mutate(percentage = n/total)
data_availability
# NA means that accession number was given in the paper but could not find on NCBI
```
...
title_row <- "Gene & Region & ID & Area & Ecosystem & Substrate & Bioproject & DOI \\\\\n"
